    ...
    <div class="container-fluid">
      ...
      <div class="card">
        <div class="card-header">
          <i class="fa fa-align-justify"></i> Gestión de Instalaciones
     ...
     <!-- contenido servicio -->
      <div>          
              <span class="md-caption">Materiales Utilizados</span>
              ...
              <div class="table-responsive col-md-12">
                <table class="table table-bordered table-striped table-sm">
                  ...
                  <tbody v-if="arrayMateriales.length">
                    <tr
                      v-for="(objeto, index) in arrayMateriales"
                      :key="`objeto-${index}`"
                    >
                      <td v-text="objeto.nombre"></td>
                      <td><input  v-model="objeto.cant" type="number" class="col-xs-2"></td>
                  ...
                  <tbody v-else>
                    <tr>
                      <td colspan="5">
                        NO hay Materiales asignados al servicio
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div class="modal-body">
            <div class="form-group row">
              <div class="col-md-9">
                <div class="input-group">
                  <select
                    class="form-control col-md-4"
                    v-model="idTpMaterial"
                    @change="listarRefM(1, this.idTpMaterial, criterio)"
                  >
                    <option
              ...
              <input
                  type="text"
                  v-model="criterio"
                  @keypress="listarRefM(1,this.idTpMaterial,criterio)"
                  class="form-control"
                  placeholder="Texto a buscar"
                >
                  <button
                    type="submit"
                    @click="listarRefM(1, this.idTpMaterial, criterio)"
                    class="btn btn-primary"
                  >
                    <i class="fa fa-search"></i> Buscar
                  </button>
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-striped table-sm">
                ...
                <tbody v-if="arrayMaterial.length">
                  <tr
                    v-for="(objeto, index) in arrayMaterial"
                    :key="`objeto-${index}`"
                  >
                ...
                <tbody v-else>
                  <tr>
                    <td colspan="5">NO hay Materiales asignados.</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
export default {
  ...
  methods: {
    ...
    abrirModal2() {
      ...
      this.getTpMat();
      this.modal2 = 1;
      this.tituloModal = "Seleccione uno o varios Materiales";
    },
    cerrarModal2() {
      this.modal2 = 0;
      this.tituloModal = "";
    ...
    getTpMat() {
      ...
      var url = "/objpqrs/selectCat?buscar=" + this.idTpMaterial;
      axios
        .get(url)
        .then(function(response) {
          //console.log(response);
    ...
    agregarMaterial(data = []) {
      ...
      if (me.encuentra3(data["id"])) {
        swal({
          type: "error",
          title: "Error...",
          text: "El Material seleccionado ya se encuentra agregado!"
        });
      } else {
        me.arrayMateriales.push({
          id: data["id"],
          nombre: data["nombre"],
          cant: data["cant"],
          und_med: data["und_med"]
        });
      }
    },
    encuentra3(id) {
      var sw = 0;
      for (var i = 0; i < this.arrayMateriales.length; i++) {
        if (this.arrayMateriales[i].id == id) {
          sw = true;
        }
      }
      return sw;
    ...
    listarRefM(page, buscar, criterio) {
      let me = this;
      var url =
      ...
      axios
        ...
        .then(function(response) {
          var respuesta = response.data;
          me.arrayMaterial = respuesta.refM.data;
          me.pagination = respuesta.pagination;
        })
        .catch(function(error) {
          console.log(error);
    ...
    cambiarPaginaM(page, buscar, criterio) {
      ...
      me.pagination.current_page = page;
      //Envia la petición para visualizar la data de esa página
      me.listarRefM(page, buscar, criterio);
    },
    registrarTicket() {
      let me = this;
  ...
  mounted() {
    // this.listarRefM(1, this.buscar, this.criterio);
    // this.getTpMaterial();
