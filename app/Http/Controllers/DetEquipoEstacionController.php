...
class DetEquipoEstacionController extends Controller
{
    ...
    public function indexEstacion(Request $request)
    {
        ...
        if ($buscar==''){
            ...
            ->join('mpios','det_equipos_estacion.id_mpio','=','mpios.id')            
            ->join('Dptos','mpios.id_dpto','=','dptos.id')            
            ->select('tp_estacions.id as idtpes','tp_estacions.nombre as tpnombre','estaciones.id as idEs','estaciones.nombre','estaciones.descripcion','dptos.id as idDpto','det_equipos_estacion.id_mpio') 
            ->distinct()           
